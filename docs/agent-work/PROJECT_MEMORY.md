...
- The Skills section groups the user's complete distinct LinkedIn skill set and additional resume-listed programming languages, frameworks, infrastructure, adaptability, and Indonesian into AI, architecture, software/data/cloud, domain, people, and language groups. Duplicates are shown once. The AI group includes neuroscience-informed memory systems and LLM/neural-network research.
- `content/IT Certificate Combined - Razi.pdf` has 20 certificate pages, rendered and optimized as individually attributed images in `public/certificates/`. The portfolio shows two larger previews; `certificates.html` lists all 20 large thumbnails with title and issuer and links each image at full size. Metadata is shared through `src/certificates-data.ts`.
- Site identity is “AI Researcher & Developer”; hero specialties focus on AI research, neuroscience-informed memory, and LLM research. The signal strip names AutoDev and Arcade.
...
- The local preview at `http://127.0.0.1:4173/` was stopped after final browser inspection at the user's request; port 4173 was confirmed free. The user's edit to `content/contentfromrazi.md`, supplied certificate PDF, and temporary `tmp/` folder remain unstaged and untouched.
